...
// Um slide de estudo (alternando claro/escuro).
function studySlide(article, idx, total, light) {
  const bg   = light ? 'bg-light' : 'bg-dark';
  const tagC = light ? 'onlight' : 'ondark';
...
// Monta o HTML completo do carrossel diário. Retorna { html, totalSlides }.
function buildDailyCarouselHtml(articles, opts = {}) {
  ...
  const capa = `
    <div class="slide bg-dark pad" style="justify-content:center;">
      ${watermark('d')}
      <div style="text-align:center;width:100%;display:flex;flex-direction:column;align-items:center;gap:16px;">
        <span class="tag gold">Ciência do dia</span>
        ${toothSvg('sym', '#37D7E7', '#EABF48')}
        <div class="word serif"><span class="a">Odonto</span><span class="b">Feed.</span></div>
        <h1 class="big serif" style="color:#EAF2F5;">${escapeHtml(dataLonga)}</h1>
        <p class="body muted-d" style="max-width:300px;">${list.length} ${list.length === 1 ? 'estudo selecionado' : 'estudos selecionados'} e resumidos pra sua clínica.</p>
      </div>
      ${progress(0, total, false)}
      ${arrow(false)}
    </div>`;

  const estudos = list.map((a, i) => studySlide(a, i + 1, total, i % 2 === 0)).join('');
