...
const COMISSAO_MENSAL = 10.00;   // R$ por assinante Premium ativo indicado
const PREMIUM_PRECO   = 59.90;   // R$ do plano (referência p/ painel)
...
// Valor a pagar POR esta indicação no mês corrente.
function valorMesAtual(u, hojeISO) {
  return statusIndicacao(u, hojeISO).status === 'premium_ativo' ? COMISSAO_MENSAL : 0;
}
...
function camposAtivacaoPremium(hojeISO) {
  const hoje = String(hojeISO || new Date().toISOString()).slice(0, 10);
...
function desempenhoAfiliado(indicados, hojeISO) {
  ...
  for (const u of indicados) {
    ...
    if (st === 'premium_ativo') { r.premiumAtivos++; r.comissaoMesAtual += COMISSAO_MENSAL; }
    ...
    // teto do status: cancelada usa os meses até hoje como estimativa).
    ...
    if (!isNaN(ini.getTime()) && st !== 'gratuito') {
      ...
      r.comissaoAcumuladaPaga += meses * COMISSAO_MENSAL;
    }
  }
...
function relatorioMensal(afiliados, hojeISO) {
  const linhas = [];
  let totalGeral = 0, totalPremiums = 0;
  for (const a of afiliados) {
    ...
    const valor = Number((ativos.length * COMISSAO_MENSAL).toFixed(2));
    if (!ativos.length) continue; // planilha só lista quem tem algo a receber
    linhas.push({ codigo: a.codigo, nome: a.nome || '', email: a.email || '', premiumsAtivos: ativos.length, valor });
    totalGeral += valor; totalPremiums += ativos.length;
  }
  linhas.sort((x, y) => y.valor - x.valor || String(x.nome).localeCompare(String(y.nome), 'pt-BR'));
  return { linhas, totalPremiums, totalGeral: Number(totalGeral.toFixed(2)) };
}
...
function relatorioCSV(rel, mes) {
  ...
  const out = [`Relatório de comissões — ${mes}`, 'Afiliado;E-mail;Código;Premiums ativos indicados;Valor a pagar (R$)'];
  for (const l of rel.linhas) {
    out.push([esc(l.nome), esc(l.email), l.codigo, l.premiumsAtivos, money(l.valor)].join(';'));
  }
  out.push(`TOTAL GERAL;;;${rel.totalPremiums};${money(rel.totalGeral)}`);
  return out.join('\r\n');
}

module.exports = {
  COMISSAO_MENSAL, PREMIUM_PRECO, DIAS_COMISSAO,
